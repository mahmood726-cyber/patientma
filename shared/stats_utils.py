...
def normal_ppf_approx(p):
    ...
    Accurate to ~1.15e-9 for 0 < p < 1 — within IEEE 754 double precision
    for all practical evidence-synthesis use cases. Matches scipy.stats.norm.ppf
    to 9 decimal places.

    Reference: Peter J. Acklam, "An algorithm for computing the inverse
    normal cumulative distribution function" (2003).

    Replaced the prior Abramowitz & Stegun 26.2.23 short-form on 2026-05-06
    after the 8-persona blinded review (review-findings-session-2026-05-06.md
    P0-1) found ppf_0.975=1.960395 vs canonical 1.9599639845 — a 4.3e-4
    discrepancy that biased every downstream 95% CI by ~0.022%. The A&S
    short-form is documented as accurate to ~4.5e-4; that's a research-
    correctness gap when the consumer wants Wichura-grade accuracy.
    """
    if p <= 0 or p >= 1:
